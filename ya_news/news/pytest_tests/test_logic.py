...
def test_unauthorized_user_cant_create_comment(
    client,
    news,
    post_comment
):
...
def test_user_cant_use_bad_words(
    ...
    comment_with_bad_words = {
        'text': f'Tests is the most {BAD_WORDS[0]} boring topic!'
    }
...
def test_authorized_user_cant_delete_comment_of_another_user(
        auth_user_client,
        comment
):
    delete_comment_url = reverse('news:delete', args=(comment.id,))
...
def test_authorized_user_cant_edit_comment_of_another_user(
        auth_user_client,
        comment,
        post_comment
):
